...
class Swagger(Model):
    ...
    host = StringType(serialize_when_none=False)  # The host (name or ip) serving the API. This MUST be the host only and does not include the scheme nor sub-paths. It MAY include a port. If the host is not included, the host serving the documentation is to be used (including the port). The host does not support path templating.
    basePath = StringType(serialize_when_none=False)  # The base path on which the API is served, which is relative to the host. If it is not included, the API is served directly under the host. The value MUST start with a leading slash (/). The basePath does not support path templating.
    ...
    securityDefinitions = DictType(SecuritySchemeType(), serialize_when_none=False)  # Security scheme definitions that can be used across the specification.
