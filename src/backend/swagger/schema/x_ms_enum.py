...
class XmsEnumValue(Model):
    ...
    description = StringType(serialize_when_none=False)  #  Property value is mandatory and corresponds to the value one would also have specified using enum
    name = StringType(serialize_when_none=False)  # allows overriding the name of the enum value that would usually be derived from the value.
...
class XmsEnum(Model):
    ...
    modelAsString = BooleanType(default=False, serialize_when_none=False)  # When set to true the enum will be modeled as a string. No validation will happen. When set to false, it will be modeled as an enum if that language supports enums. Validation will happen, irrespective of support of enums in that language.
    values = ListType(ModelType(XmsEnumValue), serialize_when_none=False)
